...
public class ItemConjureChicken extends Item {

	public ItemConjureChicken() 
	{
		super();
	}
	...
	{		
		if(!worldIn.isRemote && worldIn.getBlockState(pos).getBlock() == NecroBlocks.ALTER)// && References.isAcceptableAlter(worldIn, pos) ) && !worldIn.isDaytime()) //runs if server
		{
			...
			EntityConjuredChicken entity = new EntityConjuredChicken(worldIn);
            ...
            entity.setCustomNameTag(playerIn.getDisplayNameString() + "'s Summoned Chicken");
		}
    ...
    {
    	if(!playerIn.worldObj.isRemote && target instanceof EntityConjuredChicken)
    	{
            ...
            entity.setCustomNameTag(playerIn.getDisplayNameString() + "'s Summoned Baby Chicken");
