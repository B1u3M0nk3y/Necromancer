...
public class ItemConjureZombie extends Item {

	public ItemConjureZombie() 
	{
		super();
	}
	...
	{		
		if(!worldIn.isRemote && worldIn.getBlockState(pos).getBlock() == NecroBlocks.ALTER) //&& References.isAcceptableAlter(worldIn, pos) )//&& !worldIn.isDaytime()) //runs if server
		{
			...
			EntityConjuredZombie entity = new EntityConjuredZombie(worldIn);
            ...
            entity.setCustomNameTag(playerIn.getDisplayNameString() + "'s Summoned Zombie");
		}
    ...
    {
    	if(!playerIn.worldObj.isRemote && target instanceof EntityConjuredZombie)
    	{
            ...
            entity.setCustomNameTag(playerIn.getDisplayNameString() + "'s Summoned Baby Zombie");
